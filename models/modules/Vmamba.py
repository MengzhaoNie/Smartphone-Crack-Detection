...
try:
    from .vmamba_cross_scan import cross_scan_fn, cross_merge_fn
except ImportError:
    from models.modules.vmamba_cross_scan import cross_scan_fn, cross_merge_fn

try:
    from .vmamba_selective_scan import selective_scan_fn, selective_scan_flop_jit
except ImportError:
    from models.modules.vmamba_selective_scan import selective_scan_fn, selective_scan_flop_jit

# FLOPs counter not prepared fro mamba2
try:
    from .vmamba_mamba2.ssd_minimal import selective_scan_chunk_fn
except ImportError:
    from models.modules.vmamba_mamba2.ssd_minimal import selective_scan_chunk_fn
...
class VSSM(nn.Module):
    ...
    def get_features(self, x):
        ...
        # 第二阶段
        # downsample1 
        x = self.layers[0].downsample(x)  # [B, C*2, H/2, W/2]
        C = x.shape[1]
        H, W = x.shape[-2:]  
        ...
        # 第三阶段
        # downsample2 
        x = self.layers[1].downsample(x)  # [B, C*2, H/2, W/2]
        C = x.shape[1]
        H, W = x.shape[-2:]  
        # blocks3处理
        ...
        # 第四阶段
        x = self.layers[2].downsample(x)  # [B, C*2, H/2, W/2]
        C = x.shape[1]
        H, W = x.shape[-2:]  
        # blocks4处理
...
def vmamba_base_s2l15(channel_first=True):
    return VSSM(
        depths=[2, 2, 5, 2], dims=64, drop_path_rate=0.3, 
        patch_size=4, in_chans=3, num_classes=1000, 
        ssm_d_state=1, ssm_ratio=0.0, ssm_dt_rank="auto", ssm_act_layer="silu",
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0, 
        ssm_init="v0", forward_type="v05_noz", 
        mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
        patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"), 
