...
def _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, dt_softplus=False, dt_limit=(0.0, float('inf'))):
    ...
    states, final_states = _state_passing_fwd(rearrange(states, '... p n -> ... (p n)'), dA_cumsum[:, :, :, -1], initial_states=rearrange(initial_states, '... p n -> ... (p n)') if initial_states is not None else None, seq_idx=seq_idx, chunk_size=chunk_size, out_dtype=C.dtype)
    states, final_states = [rearrange(t, '... (p n) -> ... p n', n=dstate) for t in [states, final_states]]
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    out, out_x = _chunk_scan_fwd(CB, x, dt, dA_cumsum, C, states, D=D, z=z, seq_idx=seq_idx)
    return (out, out_x, dt, dA_cumsum, states, final_states)

def _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, dfinal_states=None, seq_idx=None, dt_softplus=False, dt_limit=(0.0, float('inf')), dx=None, ddt=None, dB=None, dC=None, dz=None, recompute_output=False):
    if dout.stride(-1) != 1:
    ...
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    states = _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=seq_idx, states_in_fp32=True)
    states, _ = _state_passing_fwd(rearrange(states, '... p n -> ... (p n)'), dA_cumsum[:, :, :, -1], initial_states=rearrange(initial_states, '... p n -> ... (p n)') if initial_states is not None else None, seq_idx=seq_idx, chunk_size=chunk_size)
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    if dout.stride(-1) != 1:
        dout = dout.contiguous()
    _, ddt, dA, *rest = selective_scan.bwd(x, dt.to(dtype=x.dtype), A, B, C, D, z, None, dout, intermediate, out, None, False, False)
...
def mamba_chunk_scan(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    states = rearrange(state_passing(rearrange(states, '... p n -> ... (p n)'), dA_cumsum[:, :, :, -1])[0], '... (p n) -> ... p n', n=dstate)
...
def ssd_chunk_scan_combined_ref(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    states = rearrange(state_passing_ref(rearrange(states, '... p n -> ... (p n)'), dA_cumsum[:, :, :, -1])[0], '... (p n) -> ... p n', n=dstate)
...
def mamba_conv1d_scan_ref(xBC, conv1d_weight, conv1d_bias, dt, A, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float('inf')), activation='silu', headdim=None, ngroups=1):
    ...
    if z is not None:
        ...
        if D.dim() == 1:
            ...
            headdim = D.shape[1]
        dim = nheads * headdim
    xBC = rearrange(causal_conv1d_fn(rearrange(xBC, 'b s d -> b d s'), conv1d_weight, conv1d_bias, activation=activation), 'b d s -> b s d')
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float('inf')), return_final_states=False, activation='silu', rmsnorm_weight=None, rmsnorm_eps=1e-06, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
        ...
        xBC_conv = rearrange(causal_conv1d_cuda.causal_conv1d_fwd(rearrange(xBC, 'b s d -> b d s'), conv1d_weight, conv1d_bias, seq_idx, None, None, activation in ['silu', 'swish']), 'b d s -> b s d')
        ...
        if rmsnorm_weight is None:
            ...
            out, _, rstd = _layer_norm_fwd(x_rms, rmsnorm_weight, None, rmsnorm_eps, z_rms, out=out, group_size=dim // ngroups, norm_before_gate=norm_before_gate, is_rms_norm=True)
            if d_nonssm == 0:
        ...
        ctx.save_for_backward(zxbcdt, conv1d_weight, conv1d_bias, out_x, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias)
        ctx.dt_limit = dt_limit
        ctx.return_final_states = return_final_states
    ...
    def backward(ctx, dout, *args):
        ...
        xBC_conv = rearrange(causal_conv1d_cuda.causal_conv1d_fwd(rearrange(xBC, 'b s d -> b d s'), conv1d_weight, conv1d_bias, seq_idx, None, None, ctx.activation in ['silu', 'swish']), 'b d s -> b s d')
        ...
        if rmsnorm_weight is None:
            ...
            dx, ddt, dA, dB, dC, dD, dz, ddt_bias, dinitial_states, *rest = _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, ctx.chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx, ddt=ddt_given, dB=dB, dC=dC, dz=dz, recompute_output=recompute_output)
            ...
            dx, ddt, dA, dB, dC, dD, _, ddt_bias, dinitial_states = _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, ctx.chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states, dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx, ddt=ddt_given, dB=dB, dC=dC)
        if outproj_weight is not None:
        ...
        dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(rearrange(xBC, 'b s d -> b d s'), conv1d_weight, conv1d_bias, rearrange(dxBC, 'b s d -> b d s'), seq_idx, None, None, dxBC_given, False, ctx.activation in ['silu', 'swish'])
...
def mamba_split_conv1d_scan_combined(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float('inf')), return_final_states=False, activation='silu', rmsnorm_weight=None, rmsnorm_eps=1e-06, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
...
def mamba_split_conv1d_scan_ref(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, dt_limit=(0.0, float('inf')), activation='silu', rmsnorm_weight=None, rmsnorm_eps=1e-06, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    if D.dim() == 1:
        assert headdim is not None
        nheads, = D.shape
    else:
    ...
    C = rearrange(C, 'b l (g n) -> b l g n', g=ngroups)
    z = rearrange(z, 'b l (h p) -> b l h p', h=nheads)
    out = ssd_selective_scan(x, dt.to(x.dtype), A, B, C, D=D.float(), z=z if rmsnorm_weight is None else None, dt_bias=dt_bias, dt_softplus=True, dt_limit=dt_limit)
    out = rearrange(out, 'b s h p -> b s (h p)')
    if rmsnorm_weight is not None:
        out = rmsnorm_fn(out, rmsnorm_weight, None, z=rearrange(z, 'b l h p -> b l (h p)'), eps=rmsnorm_eps, norm_before_gate=norm_before_gate)
    if outproj_weight is not None:
        out = F.linear(out, outproj_weight, outproj_bias)
    return out